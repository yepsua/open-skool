...
use OpenSkool\AdminBundle\Entity\Facultad;
use OpenSkool\AdminBundle\Form\FacultadType;
...
class FacultadController extends Controller
{
    ...
    {
        $grid = new Grid('facultad','list.view.grid.title');
        $grid->setUrl($this->generateUrl('facultad_data'));
        $grid->setTranslator($this->get('translator'), 'OpenSkoolAdminBundle_Facultad');
        $grid->createView();
        $grid->setEntityManager($this->getDoctrine()->getManager());
        
        $fields = array(
          'facultad.id' => array('hidden' => true),      
          'facultad.codigo' => 'Codigo',      
          'facultad.nombre' => 'Nombre',              
          'instituto.id' => array('title' => 'Instituto', 'association' => 'OpenSkool\AdminBundle\Entity\Instituto'),        
          'carrera.id' => array('title' => 'Carrera', 'association' => 'OpenSkool\AdminBundle\Entity\Carrera'),  
        );
    ...
    /**
     * Public service - All Facultad entities
     * @Route("/data", name="facultad_data")
     * @Method("GET")
     */
    public function dataAction()
    {
        try{
            ...
            $repository = $em->getRepository('OpenSkoolAdminBundle:Facultad');
            $count = Dao::count($repository);
            
            if($count > 0){
                $query = Dao::buildQuery($repository, 'facultad', $orderBy, $sord, $filters);
                $query = $query->leftJoin('facultad.instituto','instituto');
                $query = $query->leftJoin('facultad.carrera','carrera');
     ...
     *
     * @Route("/create", name="facultad_create")
     * @Method("POST")
     * @Template("OpenSkoolAdminBundle:Facultad:new.html.twig")
     */
        ...
        try{
            ...
            if ($form->isValid()) {
                ...
                if($this->getRequest()->get('_loop_create')){
                    $form = $this->createForm(new FacultadType(), new Facultad());
                    return $this->render('OpenSkoolAdminBundle:Facultad:new.html.twig',array(
                      'entity'      => $entity,
                      'form' => $form->createView())
                    ); 
                }else{
                    $deleteForm = $this->createDeleteForm($entity->getId());
                    return $this->render('OpenSkoolAdminBundle:Facultad:show.html.twig',array(
                      'entity' => $entity,
    ...
    public function showAction($id)
    {
        try{
            ...
            $entity = $em->getRepository('OpenSkoolAdminBundle:Facultad')->find($id);
     ...
     * @Method("PUT")
     * @Template("OpenSkoolAdminBundle:Facultad:edit.html.twig")
     */
    public function updateAction(Request $request, $id)
    {
        try{
            ...
            return $this->render('OpenSkoolAdminBundle:Facultad:edit.html.twig', array(
                'entity'      => $entity,
                'edit_form'   => $editForm->createView(),
                'delete_form' => $deleteForm->createView(),
            ), new Response(null, 202));
    ...
    public function deleteAction(Request $request, $id)
    {
        try{
            ...
            if ($form->isValid()) {
                $em = $this->getDoctrine()->getManager();
                $entities = $em->getRepository('OpenSkoolAdminBundle:Facultad')->findById($id);
